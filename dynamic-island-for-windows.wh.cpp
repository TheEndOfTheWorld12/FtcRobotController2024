...
// Horizontal margin the window carries around the pill. The shift buttons reach
// into it, so it sets how far out they can go — with room to spare for the
// 2.5% the pill inflates by under the pointer, which grows into the same space.
// Changing it does not move the pill: the anchor insets below are measured to
// the pill's own edge and this is subtracted back out of them.
constexpr float kRenderPadX = 36.0f;
// Vertical margin. Changing it does not move the pill: the anchor insets below
// are measured to the pill's own edge and this is subtracted back out of them.
constexpr float kRenderPadY = 22.0f;
// Where the pill's own edges sit, from the edge of the work area, at the
// anchors that measure to them.
constexpr int kPillEdgeInsetTop = 30;
constexpr int kPillEdgeInsetBottom = 62;
constexpr int kPillEdgeInsetX = 44;
...
// contents are laid out in does not move, so nothing else in the mod — least
// of all the height it reads to tell a collapsed pill from an expanded one —
// notices they are there.
constexpr float kShiftBarWidth = 25.0f;
constexpr float kShiftBarMargin = 2.0f;  // from the body's left/right edge
constexpr float kShiftBarInset = 4.0f;   // from the body's top/bottom edge
constexpr float kShiftBandExtent = kShiftBarMargin + kShiftBarWidth;
...
static_assert(kShiftBandExtent <= kRenderPadX,
              "the shift buttons must fit in the margin beside the pill");
...
// One of the pill's countdowns. The end is kept as an absolute moment rather
// than a number that gets decremented every frame, so it neither drifts nor
// loses time while the overlay is rebuilt for a display change.
struct TimerEntry {
    bool running = false;
...
// Several at once. The page steps through them with its own arrows, the way
// the media page steps through audio sources; the pill shows whichever will
// ring first, because that is the one worth interrupting for.
struct TimerSnapshot {
    std::vector<TimerEntry> timers{TimerEntry{}};
...
void AnchorPointForPosition(Position position, int width, int height, int* outX, int* outY) {
    ...
    const int padX = static_cast<int>(kRenderPadX);
    ...
    switch (position) {
        case Position::TopLeft:
            x = work.left + kPillEdgeInsetX - padX;
            y = work.top + kPillEdgeInsetTop - padY;
            break;
        case Position::TopRight:
            x = work.right - width - (kPillEdgeInsetX - padX);
            y = work.top + kPillEdgeInsetTop - padY;
            break;
...
// about again until the location moves.
...
// Reads the number out of a {"unitCode":..., "value":N} pair belonging to a
// named field. Returns false for the JSON null the service writes wherever an
// instrument is missing or out of service, which is common enough that every
// caller has to cope with it.
static bool ParseNwsValue(const char* object, const char* key, double* out) {
    if (!object || !key || !out) {
        return false;
    }
    const char* found = strstr(object, key);
    ...
    const char* value = strstr(found + strlen(key), "\"value\"");
    if (!value) {
        return false;
    }
    value += 7;
...
// How long ago the observation was taken. Stations report hourly and some go
// quiet for a while, so a stale one is worse than no override at all — the
// model at least knows what time it is.
static bool NwsObservationAgeMinutes(const char* properties, double* outMinutes) {
    const char* stamp = strstr(properties, "\"timestamp\"");
    if (!stamp) {
        return false;
    }
    stamp = strchr(stamp + 11, '"');
    if (!stamp) {
        return false;
    }
    int year = 0, month = 0, day = 0, hour = 0, minute = 0;
    if (sscanf(stamp + 1, "%d-%d-%dT%d:%d", &year, &month, &day, &hour, &minute) != 5) {
        return false;
    }

    SYSTEMTIME taken = {};
    taken.wYear = static_cast<WORD>(year);
    taken.wMonth = static_cast<WORD>(month);
    taken.wDay = static_cast<WORD>(day);
    taken.wHour = static_cast<WORD>(hour);
    taken.wMinute = static_cast<WORD>(minute);

    SYSTEMTIME now = {};
    GetSystemTime(&now);

    FILETIME takenFt = {}, nowFt = {};
    if (!SystemTimeToFileTime(&taken, &takenFt) || !SystemTimeToFileTime(&now, &nowFt)) {
        return false;
    }
    ULARGE_INTEGER a = {}, b = {};
    a.LowPart = takenFt.dwLowDateTime;
    a.HighPart = takenFt.dwHighDateTime;
    b.LowPart = nowFt.dwLowDateTime;
    b.HighPart = nowFt.dwHighDateTime;
    if (b.QuadPart < a.QuadPart) {
        *outMinutes = 0.0;   // a clock a minute ahead of the service is not staleness
        return true;
    }
    *outMinutes = static_cast<double>(b.QuadPart - a.QuadPart) / 600000000.0;
    return true;
}
...
// coarse.
static int NwsWeatherCode(const std::string& text) {
    std::string lower;
    ...
    if (has("snow") || has("sleet") || has("ice")) return 73;
    if (has("freezing")) return 66;
    if (has("drizzle")) return 53;
    if (has("rain") || has("shower")) return 63;
    if (has("fog") || has("mist")) return 45;
    if (has("haze") || has("smoke") || has("dust")) return 45;
    if (has("overcast")) return 3;
    if (has("mostly cloudy") || has("broken")) return 3;
    if (has("partly cloudy") || has("partly sunny") || has("scattered")) return 2;
    if (has("mostly clear") || has("mostly sunny") || has("few")) return 1;
    if (has("clear") || has("sunny") || has("fair")) return 0;
    return -1;
}

// The stations nearest a position, nearest first, and whether the service
// covers the position at all. Only the weather thread touches this.
struct NwsStations {
...
static void ResolveNwsStations(double latitude, double longitude) {
    const bool moved = std::fabs(latitude - g_nwsStations.latitude) > 0.02 ||
                       std::fabs(longitude - g_nwsStations.longitude) > 0.02;
    if (!moved && (!g_nwsStations.ids.empty() || g_nwsStations.unavailable)) {
        return;
    }

    g_nwsStations = NwsStations();
    g_nwsStations.latitude = latitude;
    g_nwsStations.longitude = longitude;

    wchar_t url[128] = {};
    swprintf_s(url, L"/points/%.4f,%.4f", latitude, longitude);
    const std::string points = HttpGet(L"api.weather.gov", url, true);

    // The grid the position falls in names its own station list, so the path
    // is taken from the answer rather than rebuilt out of the three fields it
    // was assembled from.
    const char* link = points.empty() ? nullptr : strstr(points.c_str(), "\"observationStations\"");
    const char* path = link ? strstr(link, "/gridpoints/") : nullptr;
    if (!path) {
        g_nwsStations.unavailable = true;
        Wh_Log(L"Weather: no US weather service coverage at %.4f,%.4f; keeping the model's "
               L"readings.", latitude, longitude);
        return;
    }
    ...
    const std::string list = HttpGet(L"api.weather.gov", widePath.c_str(), true);
    const char* cursor = list.c_str();
    while ((cursor = strstr(cursor, "\"stationIdentifier\"")) != nullptr) {
        cursor = strchr(cursor + 19, '"');
        if (!cursor) break;
        ++cursor;
        const char* stop = strchr(cursor, '"');
        if (!stop) break;
        g_nwsStations.ids.emplace_back(cursor, stop - cursor);
        cursor = stop;
    }

    if (g_nwsStations.ids.empty()) {
        g_nwsStations.unavailable = true;
        return;
    }
    Wh_Log(L"Weather: nearest reporting station is %S.", g_nwsStations.ids.front().c_str());
}

static bool FetchNwsObservation(double latitude, double longitude, bool isImperial,
                                NwsObservation* out) {
    if (!out) {
        return false;
    }
    ResolveNwsStations(latitude, longitude);
    if (g_nwsStations.unavailable || g_nwsStations.ids.empty()) {
        return false;
    }

    for (const std::string& id : g_nwsStations.ids) {
        std::string path = "/stations/" + id + "/observations/latest";
        std::wstring widePath(path.begin(), path.end());
        const std::string response = HttpGet(L"api.weather.gov", widePath.c_str(), true);
        if (response.empty()) {
            continue;
        }
        const char* properties = strstr(response.c_str(), "\"properties\"");
        if (!properties) {
            continue;
        }

        double celsius = 0.0;
        if (!ParseNwsValue(properties, "\"temperature\"", &celsius)) {
            continue;   // the station is up but has no thermometer reporting
        }

        double age = 0.0;
        if (NwsObservationAgeMinutes(properties, &age) && age > 90.0) {
            continue;   // reporting, but not lately
        }

        auto toDisplay = [isImperial](double c) { return isImperial ? c * 9.0 / 5.0 + 32.0 : c; };

        NwsObservation reading;
        reading.hasTemperature = true;
        reading.temperature = toDisplay(celsius);

        wchar_t buf[32] = {};
        double value = 0.0;

        // Heat index and wind chill are the service's own "feels like", and at
        // most one of them is ever set. Each is taken only in the direction it
        // is for: below about eighty degrees the heat index formula returns
        // numbers cooler than the air, which is not what "feels like" means to
        // anyone reading it.
        double feels = celsius;
        if (ParseNwsValue(properties, "\"heatIndex\"", &value) && value > celsius) {
            feels = value;
        } else if (ParseNwsValue(properties, "\"windChill\"", &value) && value < celsius) {
            feels = value;
        }
        swprintf_s(buf, L"%.0f", toDisplay(feels));
        reading.feelsLike = buf;

        if (ParseNwsValue(properties, "\"relativeHumidity\"", &value)) {
            swprintf_s(buf, L"%.0f", value);
            reading.humidity = buf;
        }
        if (ParseNwsValue(properties, "\"dewpoint\"", &value)) {
            swprintf_s(buf, L"%.0f\x00B0", toDisplay(value));
            reading.dewPoint = buf;
        }
        if (ParseNwsValue(properties, "\"windSpeed\"", &value)) {
            // Reported in km/h whatever the temperature is in.
            swprintf_s(buf, L"%.0f", isImperial ? value * 0.621371 : value);
            reading.windSpeed = buf;
        }
        if (ParseNwsValue(properties, "\"windDirection\"", &value)) {
            reading.windDir = CompassFromDegrees(value);
        }

        const char* described = strstr(properties, "\"textDescription\"");
        if (described) {
            described = strchr(described + 17, '"');
            if (described) {
                const char* stop = strchr(described + 1, '"');
                if (stop) {
                    const std::string words(described + 1, stop - described - 1);
                    if (!words.empty()) {
                        reading.weatherCode = NwsWeatherCode(words);
                        const int chars =
                            MultiByteToWideChar(CP_UTF8, 0, words.c_str(), -1, nullptr, 0);
                        if (chars > 0) {
        ...
        Wh_Log(L"Weather: %S observed %.1f%s, %s%% humidity, %.0f minutes ago.", id.c_str(),
               reading.temperature, isImperial ? L"F" : L"C", reading.humidity.c_str(), age);
        ...
        if (!wRes.empty()) {
            ...
            if (pinned || ParseWttrCoordinates(wRes.c_str(), &latitude, &longitude)) {
                ...
                if ((FetchNwsGridHour(latitude, longitude, isFahrenheit, &observed) ||
                    ...
                    if (!observed.desc.empty()) desc = observed.desc;
                    if (!observed.feelsLike.empty()) feelsLike = observed.feelsLike;
                    if (!observed.humidity.empty()) humidity = observed.humidity;
                    if (!observed.windSpeed.empty()) windSpeed = observed.windSpeed;
                    if (!observed.windDir.empty()) windDir = observed.windDir;
                }
...
void PersistTimers(const TimerSnapshot& timers) {
    const int count = static_cast<int>(timers.timers.size());
    Wh_SetIntValue(L"TimerCount", count);
...
void LoadTimers() {
    // The first timer's length used to live under its own key, from when there
    // was only ever one. It is still read so an existing setup comes back as
    // it was rather than reset to five minutes.
    const int legacy = Wh_GetIntValue(L"TimerDurationSeconds", 300);
    const int count = ClampInt(Wh_GetIntValue(L"TimerCount", 1), 1, kTimerMaxCount);

    TimerSnapshot loaded;
    loaded.timers.clear();
    for (int i = 0; i < count; ++i) {
        wchar_t key[32] = {};
        swprintf_s(key, L"TimerDuration%d", i);
        const int fallback = i == 0 ? legacy : 300;
        TimerEntry timer;
        timer.durationSeconds =
            ClampInt(Wh_GetIntValue(key, fallback), kTimerMinSeconds, kTimerMaxSeconds);
        timer.remaining = timer.durationSeconds;
        loaded.timers.push_back(timer);
    }
...
double TimerRemainingSeconds(const TimerEntry& timer, double now) {
    if (!timer.running) {
...
// with the least time left, since that is the next thing to happen; and with
// none running, whichever the page is sitting on.
int TimerToShow(const TimerSnapshot& timers, double now) {
    int soonest = -1;
    double least = 0.0;
    for (size_t i = 0; i < timers.timers.size(); ++i) {
        const TimerEntry& timer = timers.timers[i];
        if (timer.finished) {
        ...
        const double left = TimerRemainingSeconds(timer, now);
        if (soonest < 0 || left < least) {
            soonest = static_cast<int>(i);
            least = left;
        }
    }
    return soonest >= 0 ? soonest : TimerSelectedIndex(timers);
}

// The list is never empty — it starts with one and Delete refuses to take the
// last — but every reader indexes into it, so the two that matter say what
// happens if it ever were rather than leaving it to chance.
const TimerEntry& TimerShownEntry(const TimerSnapshot& timers, double now) {
    static const TimerEntry kNone;
    if (timers.timers.empty()) {
        return kNone;
    }
    return timers.timers[TimerToShow(timers, now)];
}

const TimerEntry& TimerSelectedEntry(const TimerSnapshot& timers) {
    static const TimerEntry kNone;
    if (timers.timers.empty()) {
        return kNone;
    }
    return timers.timers[TimerSelectedIndex(timers)];
}

bool AnyTimerActive(const TimerSnapshot& timers) {
    for (const TimerEntry& timer : timers.timers) {
...
void TimerSelect(int delta) {
    ...
    int chosen = (TimerSelectedIndex(g_state.timer) + delta) % count;
    if (chosen < 0) {
        chosen += count;
    }
    g_state.timer.selected = chosen;
}

void TimerAdd() {
    std::lock_guard lock(g_stateMutex);
    if (static_cast<int>(g_state.timer.timers.size()) >= kTimerMaxCount) {
        return;
    }
...
void TimerRemove() {
    std::lock_guard lock(g_stateMutex);
    if (g_state.timer.timers.size() < 2) {
        return;  // the list is never empty; there is nothing to fall back to
    }
...
class Renderer {
   ...
   private:
    ...
    void DrawPill(const SharedState& state, const Settings& settings, const Activity& activity,
        ...
        // — goes on the body, which is the pill's rect grown sideways by the
        // shift band when a resting pill is carrying the buttons. The contents
        // keep the rect itself, so their layout is untouched and the band costs
        // nothing but paint. Growing it widthways also leaves the pill's height
        // alone, so its corner radius and whole silhouette are unchanged. The
        // band is in device pixels, like the margin it reaches into, so it does
        // not take the size scale.
        //
        // Clamped to the room left on the bitmap, because the hover inflation
        // grows into the same margin the band wants. The margin is sized to
        // hold both at any ordinary size scale; past that the buttons narrow
        // rather than being cut off by the edge of the window.
        const float roomX = std::min(rect.left, static_cast<float>(bitmapWidth_) - rect.right) - 1.0f;
        const float band = std::max(0.0f, std::min(shiftBand, roomX));
        const D2D1_RECT_F body =
            band > 0.01f
                ? D2D1::RectF(rect.left - band, rect.top, rect.right + band, rect.bottom)
                : rect;
        ...
        if (band > 0.01f) {
            DrawShiftBars(body, band, shiftBand / kShiftBandExtent);
        }
    }
    ...
    // Page: the countdown the page is sitting on, how much of it has gone, and
    // two rows of buttons — the first drives this countdown, the second manages
    // the list. Every button publishes the rectangle it was drawn at, as every
    // other control in the pill does, so the hit-test cannot disagree with the
    // paint.
    void DrawTimerDashboard(const SharedState& state, D2D1_RECT_F rect, double now) {
        const int count = static_cast<int>(state.timer.timers.size());
        const int index = TimerSelectedIndex(state.timer);
        const TimerEntry& timer = TimerSelectedEntry(state.timer);
        ...
        const TimerButtonSpec buttons[kTimerButtonCount] = {
            ...
            {L"Clock", 56.0f, false, true},
        };

        const float gap = 8.0f;
        const float pcx = (rect.left + rect.right) * 0.5f;
        const float pcy = (rect.top + rect.bottom) * 0.5f;
        const float rowTop[2] = {rect.top + 113.0f, rect.top + 141.0f};
        ...
        for (int row = 0; row < 2; ++row) {
            ...
            for (int slot = 0; slot < members; ++slot) {
                ...
                label->SetOpacity(spec.enabled ? (isHovered ? 1.0f : 0.86f) : 0.28f);
                ...
                target_->DrawTextW(spec.label, static_cast<UINT32>(wcslen(spec.label)),
                                   smallTextFormat_.Get(),
                                   D2D1::RectF(box.left, box.top + 4.5f, box.right,
                                               box.bottom + 3.0f),
    ...
    // The collapsed pill a timer owns for as long as it is counting: whichever
    // one rings first, the time left on it, and how much of it has gone. When
    // it rings the whole thing pulses.
    void DrawTimerPill(const SharedState& state, D2D1_RECT_F rect, double now) {
        ...
        const int count = static_cast<int>(state.timer.timers.size());
        const int index = TimerToShow(state.timer, now);
        const TimerEntry& timer = TimerShownEntry(state.timer, now);
        const double remaining = TimerRemainingSeconds(timer, now);
        const float pulse =
            timer.finished
    ...
    //
    // amount runs 0 to 1 as the band opens and closes. The buttons fade with
    // it, and only take clicks once they have all but arrived: a control
    // halfway out of the pill is not one to press.
    void DrawShiftBars(D2D1_RECT_F body, float band, float amount) {
        const float alpha = Clamp(amount, 0.0f, 1.0f);
        const float barWidth = std::min(kShiftBarWidth, band - kShiftBarMargin);
        if (alpha <= 0.01f || barWidth < 6.0f ||
    ...
    void DrawShiftBar(D2D1_RECT_F body, float barWidth, bool up, bool bottomAnchored,
        ...
        if (bg) {
            const float radius =
                std::min(bar.right - bar.left, bar.bottom - bar.top) * 0.5f;
            target_->FillRoundedRectangle(D2D1::RoundedRect(bar, radius, radius), bg.Get());
        }

        const float cx = (bar.left + bar.right) * 0.5f;
        const float cy = (bar.top + bar.bottom) * 0.5f;
        const float armX = 6.4f;
        const float armY = 3.7f;
...
std::vector<IslandKind> ChooseActivities(const SharedState& state, const Settings& settings, double now) {
    ...
    const bool timerActive = AnyTimerActive(state.timer);
    if (timerActive) {
...
// Whether the pointer is in the margin strip the shift buttons are drawn in.
...
// in the resting shape that shows them. The buttons run the pill's full height,
// so the strip is the whole of the margin to either side of it.
static bool PointerInShiftZone(const RECT& windowRect, POINT cursor) {
    ...
    // supposed to bring it back. Below a pill's worth of width there is nothing
    // here to shift.
    if (w < kRenderPadX * 2.0f + 24.0f || h < kRenderPadY * 2.0f) {
        return false;
    }
    const float x = static_cast<float>(cursor.x - windowRect.left);
    return x < kRenderPadX || x > w - kRenderPadX;
}
        ...
        case WM_LBUTTONUP:
            {
                ...
                // it with the button up.
                {
                    ...
                    for (TimerEntry& timer : g_state.timer.timers) {
                        if (timer.finished) {
                            timer.finished = false;
                            anyRinging = true;
                        }
                    ...
                    if (button >= 0) {
                        switch (static_cast<TimerButton>(button)) {
                            ...
                            case TimerButton::Plus:
                                TimerAdjust(kTimerStepSeconds);
                                break;
                            case TimerButton::Next:
                                TimerSelect(1);
                                break;
                            case TimerButton::Reset:
                                TimerReset();
                                break;
                            case TimerButton::New:
                                TimerAdd();
                                break;
                            case TimerButton::Delete:
                                TimerRemove();
                                break;
    ...
    for (;;) {
        ...
        while (WaitForSingleObject(g_stopEvent, 0) == WAIT_TIMEOUT) {
            ...
            bool timerJustFinished = false;
            {
                ...
                for (TimerEntry& timer : g_state.timer.timers) {
                    if (timer.running && now >= timer.endsAt) {
                        timer.running = false;
                        timer.remaining = 0.0;
                        timer.finished = true;
                    ...
                    if (timer.finished && now >= timer.finishedAt + 12.0) {
                        timer.finished = false;
                    }
                }
            ...
            // included — rather than a larger version of the chip.
            //
            // Which set of pages that is matters: the media page only exists
            // while the pill's kind is Media, so handing a counting timer back
            ...
            // than on every frame it does not.
            {
                ...
                const int timerSecond =
                    static_cast<int>(std::ceil(TimerRemainingSeconds(shownTimer, now)));
                ...
                if (now >= nextDurationWrite) {
                    ...
                    if (TimerDigest(snapshot.timer) != g_timerPersistDigest) {
                        PersistTimers(snapshot.timer);
                    }
                }
            }
            ...
            if (primary.kind == IslandKind::Media || primary.kind == IslandKind::BatteryLow ||
                ...
                (primary.kind == IslandKind::Timer &&
                 TimerShownEntry(snapshot.timer, now).finished)) {
                needsRender = true;
            }
